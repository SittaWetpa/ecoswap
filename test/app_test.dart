...
void main() {
  group('EcoSwapApp route guard', () {
    ...
    testWidgets(
        'shows LoginScreen when authStateStream emits null',
        (tester) async {
      await tester.pumpWidget(
        EcoSwapApp(authStateStream: Stream.value(null)),
      );
      await tester.pump();
    ...
    testWidgets(
        'shows loading indicator while stream is in waiting state',
        (tester) async {
      ...
      await tester.pumpWidget(
        EcoSwapApp(authStateStream: controller.stream),
      );

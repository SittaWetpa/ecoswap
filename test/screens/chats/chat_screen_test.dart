...
void main() {
  group('ChatScreen — WBS 9.2', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          _buildScreen(onSend: (_) => sendCalled = true),
        );
    ...
    testWidgets(
      'send button becomes enabled after typing text',
      (tester) async {
        String? sentText;

        await tester.pumpWidget(
          _buildScreen(onSend: (t) => sentText = t),
        );
    ...
    testWidgets(
      '"Ready to swap" hidden when 3 mine + 2 theirs (theirs < 3)',
      (tester) async {
        final messages = [
          _msg(_kCurrentUid, 'Msg 1', index: 0),
          _msg(_kCurrentUid, 'Msg 2', index: 1),
    ...
    testWidgets(
      'long messages wrap and do not overflow',
      (tester) async {
        const longText =
    ...
    testWidgets(
      'trade pill shows my item name and their item name',
      (tester) async {
    ...
    testWidgets(
      'header does not display "Active now" text',
      (tester) async {
        await tester.pumpWidget(_buildScreen());

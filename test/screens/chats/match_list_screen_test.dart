...
void main() {
  group('MatchListScreen', () {
    ...
    testWidgets(
      'user with 2 active + 1 cancelled match sees 2 rows',
      (tester) async {
    ...
    testWidgets(
      'trade pill correctly shows "Your X ⇄ their Y"',
      (tester) async {
        final controller = StreamController<List<MatchRowData>>();
        ...
        expect(
          find.text('Your Electric ⇄ their Leather'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'tap on row navigates to correct chat',
      (tester) async {
        final controller = StreamController<List<MatchRowData>>();
        ...
        controller.add([
          _fakeRow(matchId: 'match-456', otherUserName: 'Beam'),
        ]);
        await tester.pump();
    ...
    testWidgets(
      'empty state shown when user has 0 matches',
      (tester) async {
        final controller = StreamController<List<MatchRowData>>();

...
void main() {
  ...
  group('Step1NamePhoto — display name validation', () {
    ...
    testWidgets('enables Next button when name is 1 char',
      ...
      await tester.enterText(
          find.byKey(const Key('displayNameField')), 'A');
      await tester.pump();
    ...
    testWidgets('hard-caps display name at 40 chars via input formatter',
      ...
      final over40 = 'A' * 45;
      await tester.enterText(
          find.byKey(const Key('displayNameField')), over40);
      await tester.pump();
      ...
      expect(actualText.length, lessThanOrEqualTo(40),
    ...
    testWidgets('Next button disabled when name is exactly 41 chars entered',
      ...
      await tester.enterText(
          find.byKey(const Key('displayNameField')), 'A' * 41);
      await tester.pump();
    ...
    testWidgets('calls onNext after valid name is saved',
      ...
      await tester.enterText(
          find.byKey(const Key('displayNameField')), 'Nong');
      await tester.pump();
    ...
    testWidgets('skipping photo leaves photoUrl as empty string',
      ...
      await tester.enterText(
          find.byKey(const Key('displayNameField')), 'Ploy');
      await tester.pump();
  ...
  group('PhotoService — user_photos compression (WBS 5.1)', () {
    test('pickAndUpload for user_photos sends < 1 MB bytes to storage',
        () async {
      ...
      expect(uploadedSize!, lessThan(PhotoService.kMaxBytes),
